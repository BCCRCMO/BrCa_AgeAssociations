...
simsmufit <- function(x, xdf = tmadf, ydf = NULL, xvar = "age_at_diagnosis",
                      ...
                      ERbindingstatus = NULL, subgroup = NULL, ssbass = 10, ssspan = 0.2
                      ) {
  rn <- nsim
  alphalev <- alpha/2.0
  UCLn <- trunc(rn * (1 - alphalev))
...
###   xlbc <- paste(toupper(substring(xlb, 1, 1)), tolower(substring(xlb, 2)), sep = "")
  ...
  plot( xdf[valdx, xvar], xdf[valdx, x], xlab = xlbc, main = "", ylab = "", ##paste("Note: bass = ", ssbass, "span = ", ssspan),
...
###     spout <- predict(smooth.spline(ssout$x, ssout$y), x = agesv)
###     simmat[i, ] <- spout$y
      simmat[i, ] <- predict(smooth.spline(supsmu( sample(xdf[valdx, xvar]), xdf[valdx, x], bass = ssbass, span = ssspan)), x = agesv)$y
...
###   lines(supsmu(agesv, srtmat[UCLn, ], bass = 10), lty = 1, lwd = 3, type = "l", col = "grey80")
  ...
  ssout <- supsmu( xdf[valdx, xvar], xdf[valdx, x], bass = ssbass, span = ssspan)
...
###  axis(side = 2, at = c(0, 20, 40, 60, 80, 100))
    for (i in seq(rn) ) {
      ssout <- supsmu( sample(ydf[valdy, xvar]), ydf[valdy, x], bass = ssbass, span = ssspan)
      spout <- predict(smooth.spline(ssout$x, ssout$y), x = agesv)
      simmat[i, ] <- spout$y
###      lines(ssout, lty = 3, lwd = 1, col = "lightgrey")
    }
...
###     lines(agesv, srtmat[UCLn, ], lty = 1, lwd = 3, type = "l", col = "grey80")
    lines(supsmu( ydf[valdy, xvar], ydf[valdy, x], bass = ssbass, span = ssspan), lwd = 3, type = "l", col = "#AA1010")
###     legend("bottomright", legend = paste(trunc(100*(1-alpha)), "% Confidence Bounds", sep = ""),
###            lty = 1, lwd = 4, seg.len = 3, text.width = 60, col = "wheat3")
  }
}
...
set.seed(743); sapply(seq(nrow(cscvars)),
                      function(idx) {
                          simsmufit(cscvars[idx, 1], xdf = tmadf,
                                    nsim = 10000, alpha = 0.01, ssbass = ssbassin, ssspan = ssspanin,
                                    pdfOutFilep = TRUE, plotDirName = plDrNm,
                                    pdfFileNamePrefix =
                                      paste("MB09_IHC_Trends_99pct_10kSimCIs_shaded_", ssbassin, "_", ssspanin, "_", 
