...
simsmufit <- function(x, xdf = tmadf, ydf = NULL, xvar = "age_at_diagnosis",
                      ...
                      ERbindingstatus = NULL, genomicLabel = NULL, IHClabel = NULL,
                      subgroup = NULL
                      ) {
  rn <- nsim
  alphalev <- alpha/2.0
...
dimnames = list(NULL, c("GPEC_name", "ER_binding", "Gene_symbol", "IHC_name")))

crcvarsEHidxs <- match(c("er_v2nc", "her2_v2nc"), cscvars[, 1])
cscvarsEH <- cscvars[-crcvarsEHidxs, ]
...
date()
plDrNm <- "./Plots/BigSeries_WholeSeries_NoMB09_NoMBex_IHC_Trends_Individual"
if ( !file.exists(plDrNm) ) { dir.create(plDrNm) }
set.seed(473); sapply(cscvars, simsmufit, xdf = wmadf, nsim = 2000, alpha = 0.05,
                      pdfOutFilep = TRUE, plotDirName = plDrNm,
                      pdfFileNamePrefix = "BigSeries_WholeSeries_NoMB09_NoMBex_IHC_Trends_95pct_2kSimCIs_shaded_")
date()
...
set.seed(473); sapply(cscvars, simsmufit, xdf = wmadf, nsim = 10000, alpha = 0.01,
                      pdfOutFilep = TRUE, plotDirName = plDrNm,
                      pdfFileNamePrefix = "BigSeries_WholeSeries_NoMB09_NoMBex_IHC_Trends_99pct_10kSimCIs_shaded_")
date()
...
if ( !file.exists(plDrNm) ) { dir.create(plDrNm) }
icinm <- "All cases"
set.seed(473); sapply(seq(nrow(cscvars)),
                      function(idx) {
                          simsmufit(cscvars[idx, 1], xdf = wmadf,
                                    nsim = 10000, alpha = 0.01,
                                    pdfOutFilep = TRUE, plotDirName = plDrNm,
                                    pdfFileNamePrefix =
                                      paste("BigSeries_WholeSeries_NoMB09_NoMBex_IHC_Trends_99pct_10kSimCIs_shaded_",
...
### TODO 20170323:  Set up ER/HER2 and run subgroups
wmadf$BrCaEH <- rep(NA_character_, nrow(wmadf))
wmadf[which(wmadf$erposneg == "Positive" & wmadf$her2_v2nc == 3), "BrCaEH"] <- "ER+/HER2+"
wmadf[which(wmadf$erposneg == "Positive" & wmadf$her2_v2nc <= 1), "BrCaEH"] <- "ER+/HER2-"
wmadf[which(wmadf$erposneg == "Negative" & wmadf$her2_v2nc == 3), "BrCaEH"] <- "ER-/HER2+"
wmadf[which(wmadf$erposneg == "Negative" & wmadf$her2_v2nc <= 1), "BrCaEH"] <- "ER-/HER2-"
wmadf$BrCaEHf <- factor(wmadf$BrCaEH, levels =  c("ER+/HER2-", "ER+/HER2+", "ER-/HER2+", "ER-/HER2-"))

date()
for ( ici in seq( along = levels(wmadf$BrCaEHf) ) ) { ## ici <- 1

    icinm <- levels(wmadf$BrCaEHf)[ici]
    cat("\n\n", icinm, "\n\n")
    ...
    set.seed(963); sapply(seq(nrow(cscvarsEH)),
                                   function(idx) {
                                       simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = wmadf[which(wmadf$BrCaEHf == icinm), ],
                                                 nsim = 10000, alpha = 0.01,
                                                 pdfOutFilep = TRUE, plotDirName = plDrNm,
                                                 pdfFileNamePrefix =
                                                   paste("BigSeries_WholeSeries_NoMB09_NoMBex_IHC_Trends_99pct_10kSimCIs_shaded_",
                                                         gsub(" ", "_", gsub( "\\/", "", gsub( "\\-", "n", gsub("\\+", "p", icinm)))),
                                                         "_", sep = ""),
                                                 ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                                 genomicLabel = cscvarsEH[idx, "Gene_symbol"],
                                                 subgroup = icinm)
                                       } )
}
date()
...
pdf(file = "BigSeries_WholeSeries_NoMB09_NoMBex_IHC_Trends_99p9pct_40kSimCIs_shaded_v01.pdf",
    width = 8, height = 8)
par(mfrow = c(2, 2))
set.seed(473); sapply(cscvars, simsmufit, xdf = wmadf, nsim = 40000, alpha = 0.001)
dev.off()
date()
...
set.seed(473); sapply(cscvars, simsmufit, xdf = wmadf, nsim = 40000, alpha = 0.001,
                      pdfOutFilep = TRUE, plotDirName = plDrNm,
                      pdfFileNamePrefix = "BigSeries_WholeSeries_NoMB09_NoMBex_IHC_Trends_99p9pct_40kSimCIs_shaded_")
date()
...
set.seed(473); sapply(cscvars, simsmufit, xdf = wmadf, nsim = 400000, alpha = 0.0001,
                      pdfOutFilep = TRUE, plotDirName = plDrNm,
                      pdfFileNamePrefix = "BigSeries_WholeSeries_NoMB09_NoMBex_IHC_Trends_99p99pct_400kSimCIs_shaded_")
date()
...
date()
pdf(file = "BigSeries_TNP_WholeSeries_NoMB09_NoMBex_IHC_Trends_95pct_2kSimCIs_shaded_v01.pdf",
    width = 8, height = 8)
par(mfrow = c(2, 2))
set.seed(473); sapply(cscvars, simsmufit, xdf = tnmadf, nsim = 2000, alpha = 0.05)
dev.off()
date()
date()
...
date()
pdf(file = "BigSeries_Luminalp_WholeSeries_NoMB09_NoMBex_IHC_Trends_95pct_2kSimCIs_shaded_v01.pdf",
    width = 8, height = 8)
par(mfrow = c(2, 2))
set.seed(473); sapply(cscvars, simsmufit, xdf = lpmadf, nsim = 2000, alpha = 0.05)
dev.off()
date()
date()
...
### Training set and validation set graphs
tmadf <- tmadfbackupall
vmadf <- vmadfbackupall

date()

plDrNm <- "./Plots/BigSeries_TrainingSeries_AllCases_IHC_Trends_Individual"
if ( !file.exists(plDrNm) ) { dir.create(plDrNm) }

set.seed(963); sapply(seq(nrow(cscvarsEH)),
                      function(idx) {
                          simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = tmadf,
                                    nsim = 2000, alpha = 0.05,
                                    pdfOutFilep = TRUE, plotDirName = plDrNm,
                                    pdfFileNamePrefix = "BigSeries_TrainingSeries_AllCases_IHC_Trends_95pct_2kSimCIs_shaded_",
                                    ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                    genomicLabel = cscvarsEH[idx, "Gene_symbol"])
                      } )
...
plDrNm <- "./Plots/BigSeries_ValidationSeries_AllCases_IHC_Trends_Individual"
if ( !file.exists(plDrNm) ) { dir.create(plDrNm) }

set.seed(963); sapply(seq(nrow(cscvarsEH)),
                      function(idx) {
                          simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = vmadf,
                                    nsim = 10000, alpha = 0.01,
                                    pdfOutFilep = TRUE, plotDirName = plDrNm,
                                    pdfFileNamePrefix = "BigSeries_ValidationSeries_AllCases_IHC_Trends_99pct_10kSimCIs_shaded_",
                                    ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                    genomicLabel = cscvarsEH[idx, "Gene_symbol"])
                      } )
date()
...
set.seed(593); sapply(seq(nrow(cscvarsEH)),
                      function(idx) {
                          simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = tmadf,
                                    nsim = 40000, alpha = 0.001,
                                    pdfOutFilep = TRUE, plotDirName = plDrNm,
                                    pdfFileNamePrefix = "BigSeries_TrainingSeries_AllCases_IHC_Trends_99p9pct_40kSimCIs_shaded_",
                                    ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                    genomicLabel = cscvarsEH[idx, "Gene_symbol"])
                      } )
...
set.seed(563); sapply(seq(nrow(cscvarsEH)),
                      function(idx) {
                          simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = vmadf,
                                    nsim = 40000, alpha = 0.001,
                                    pdfOutFilep = TRUE, plotDirName = plDrNm,
                                    pdfFileNamePrefix = "BigSeries_ValidationSeries_AllCases_IHC_Trends_99p9pct_40kSimCIs_shaded_",
                                    ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                    genomicLabel = cscvarsEH[idx, "Gene_symbol"])
                      } )
date()
...
set.seed(961); sapply(seq(nrow(cscvarsEH)),
                      function(idx) {
                          simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = tmadf,
                                    nsim = 400000, alpha = 0.0001,
                                    pdfOutFilep = TRUE, plotDirName = plDrNm,
                                    pdfFileNamePrefix = "BigSeries_TrainingSeries_AllCases_IHC_Trends_99p99pct_400kSimCIs_shaded_",
                                    ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                    genomicLabel = cscvarsEH[idx, "Gene_symbol"])
                      } )
...
set.seed(963); sapply(seq(nrow(cscvarsEH)),
                      function(idx) {
                          simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = vmadf,
                                    nsim = 400000, alpha = 0.0001,
                                    pdfOutFilep = TRUE, plotDirName = plDrNm,
                                    pdfFileNamePrefix = "BigSeries_ValidationSeries_AllCases_IHC_Trends_99p99pct_400kSimCIs_shaded_",
                                    ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                    genomicLabel = cscvarsEH[idx, "Gene_symbol"])
                      } )
date()
...
### Set up ER/HER2 and run subgroups
tmadf$BrCaEH <- rep(NA_character_, nrow(tmadf))
tmadf[which(tmadf$erposneg == "Positive" & tmadf$her2_v2nc == 3), "BrCaEH"] <- "ER+/HER2+"
tmadf[which(tmadf$erposneg == "Positive" & tmadf$her2_v2nc <= 1), "BrCaEH"] <- "ER+/HER2-"
tmadf[which(tmadf$erposneg == "Negative" & tmadf$her2_v2nc == 3), "BrCaEH"] <- "ER-/HER2+"
tmadf[which(tmadf$erposneg == "Negative" & tmadf$her2_v2nc <= 1), "BrCaEH"] <- "ER-/HER2-"
tmadf$BrCaEHf <- factor(tmadf$BrCaEH, levels =  c("ER+/HER2-", "ER+/HER2+", "ER-/HER2+", "ER-/HER2-"))
### Set up ER/HER2 and run subgroups
vmadf$BrCaEH <- rep(NA_character_, nrow(vmadf))
vmadf[which(vmadf$erposneg == "Positive" & vmadf$her2_v2nc == 3), "BrCaEH"] <- "ER+/HER2+"
vmadf[which(vmadf$erposneg == "Positive" & vmadf$her2_v2nc <= 1), "BrCaEH"] <- "ER+/HER2-"
vmadf[which(vmadf$erposneg == "Negative" & vmadf$her2_v2nc == 3), "BrCaEH"] <- "ER-/HER2+"
vmadf[which(vmadf$erposneg == "Negative" & vmadf$her2_v2nc <= 1), "BrCaEH"] <- "ER-/HER2-"
vmadf$BrCaEHf <- factor(vmadf$BrCaEH, levels =  c("ER+/HER2-", "ER+/HER2+", "ER-/HER2+", "ER-/HER2-"))
...
for ( ici in seq( along = levels(tmadf$BrCaEHf) ) ) { ## ici <- 1
    ...
    plDrNm <- "./Plots/BigSeries_TrainingSeries_AllCases_ER_HER2_subgroups_IHC_Trends_Individual"
    if ( !file.exists(plDrNm) ) { dir.create(plDrNm) }

    set.seed(473); sapply(seq(nrow(cscvarsEH)),
                                   function(idx) {
                                       simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = tmadf[which(tmadf$BrCaEHf == icinm), ],
                                                 nsim = 2000, alpha = 0.05,
                                                 pdfOutFilep = TRUE, plotDirName = plDrNm,
                                                 pdfFileNamePrefix =
                                                   paste("BigSeries_TrainingSeries_AllCases_IHC_Trends_95pct_2kSimCIs_shaded_",
                                                         gsub(" ", "_", gsub( "\\/", "", gsub( "\\-", "n", gsub("\\+", "p", icinm)))),
                                                         "_", sep = ""),
                                                 ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                                 genomicLabel = cscvarsEH[idx, "Gene_symbol"],
...
date()
for ( ici in seq( along = levels(vmadf$BrCaEHf) ) ) { ## ici <- 1

    icinm <- levels(vmadf$BrCaEHf)[ici]
    cat("\n\n", icinm, "\n\n")

    plDrNm <- "./Plots/BigSeries_ValidationSeries_AllCases_ER_HER2_subgroups_IHC_Trends_Individual"
    if ( !file.exists(plDrNm) ) { dir.create(plDrNm) }

    set.seed(967); sapply(seq(nrow(cscvarsEH)),
                                   function(idx) {
                                       simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = vmadf[which(vmadf$BrCaEHf == icinm), ],
                                                 nsim = 2000, alpha = 0.05,
                                                 pdfOutFilep = TRUE, plotDirName = plDrNm,
                                                 pdfFileNamePrefix =
                                                   paste("BigSeries_ValidationSeries_AllCases_IHC_Trends_95pct_2kSimCIs_shaded_",
                                                         gsub(" ", "_", gsub( "\\/", "", gsub( "\\-", "n", gsub("\\+", "p", icinm)))),
                                                         "_", sep = ""),
                                                 ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                                 genomicLabel = cscvarsEH[idx, "Gene_symbol"],
    ...
    set.seed(473); sapply(seq(nrow(cscvarsEH)),
                                   function(idx) {
                                       simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = tmadf[which(tmadf$BrCaEHf == icinm), ],
                                                 ...
                                                 pdfOutFilep = TRUE, plotDirName = plDrNm,
                                                 pdfFileNamePrefix =
                                                   paste("BigSeries_TrainingSeries_AllCases_IHC_Trends_99pct_10kSimCIs_shaded_",
                                                         gsub(" ", "_", gsub( "\\/", "", gsub( "\\-", "n", gsub("\\+", "p", icinm)))),
                                                         "_", sep = ""),
                                                 ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                                 genomicLabel = cscvarsEH[idx, "Gene_symbol"],
    ...
    set.seed(967); sapply(seq(nrow(cscvarsEH)),
                                   function(idx) {
                                       simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = vmadf[which(vmadf$BrCaEHf == icinm), ],
                                                 ...
                                                 pdfOutFilep = TRUE, plotDirName = plDrNm,
                                                 pdfFileNamePrefix =
                                                   paste("BigSeries_ValidationSeries_AllCases_IHC_Trends_99pct_10kSimCIs_shaded_",
                                                         gsub(" ", "_", gsub( "\\/", "", gsub( "\\-", "n", gsub("\\+", "p", icinm)))),
                                                         "_", sep = ""),
                                                 ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                                 genomicLabel = cscvarsEH[idx, "Gene_symbol"],
    ...
    set.seed(473); sapply(seq(nrow(cscvarsEH)),
                                   function(idx) {
                                       simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = tmadf[which(tmadf$BrCaEHf == icinm), ],
                                                 ...
                                                 pdfOutFilep = TRUE, plotDirName = plDrNm,
                                                 pdfFileNamePrefix =
                                                   paste("BigSeries_TrainingSeries_AllCases_IHC_Trends_99p9pct_40kSimCIs_shaded_",
                                                         gsub(" ", "_", gsub( "\\/", "", gsub( "\\-", "n", gsub("\\+", "p", icinm)))),
                                                         "_", sep = ""),
                                                 ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                                 genomicLabel = cscvarsEH[idx, "Gene_symbol"],
    ...
    set.seed(967); sapply(seq(nrow(cscvarsEH)),
                                   function(idx) {
                                       simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = vmadf[which(vmadf$BrCaEHf == icinm), ],
                                                 ...
                                                 pdfOutFilep = TRUE, plotDirName = plDrNm,
                                                 pdfFileNamePrefix =
                                                   paste("BigSeries_ValidationSeries_AllCases_IHC_Trends_99p9pct_40kSimCIs_shaded_",
                                                         gsub(" ", "_", gsub( "\\/", "", gsub( "\\-", "n", gsub("\\+", "p", icinm)))),
                                                         "_", sep = ""),
                                                 ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                                 genomicLabel = cscvarsEH[idx, "Gene_symbol"],
    ...
    set.seed(473); sapply(seq(nrow(cscvarsEH)),
                                   function(idx) {
                                       simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = tmadf[which(tmadf$BrCaEHf == icinm), ],
                                                 ...
                                                 pdfOutFilep = TRUE, plotDirName = plDrNm,
                                                 pdfFileNamePrefix =
                                                   paste("BigSeries_TrainingSeries_AllCases_IHC_Trends_99p99pct_400kSimCIs_shaded_",
                                                         gsub(" ", "_", gsub( "\\/", "", gsub( "\\-", "n", gsub("\\+", "p", icinm)))),
                                                         "_", sep = ""),
                                                 ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                                 genomicLabel = cscvarsEH[idx, "Gene_symbol"],
    ...
    set.seed(967); sapply(seq(nrow(cscvarsEH)),
                                   function(idx) {
                                       simsmufit(cscvarsEH[idx, "GPEC_name"], xdf = vmadf[which(vmadf$BrCaEHf == icinm), ],
                                                 ...
                                                 pdfOutFilep = TRUE, plotDirName = plDrNm,
                                                 pdfFileNamePrefix =
                                                   paste("BigSeries_ValidationSeries_AllCases_IHC_Trends_99p99pct_400kSimCIs_shaded_",
                                                         gsub(" ", "_", gsub( "\\/", "", gsub( "\\-", "n", gsub("\\+", "p", icinm)))),
                                                         "_", sep = ""),
                                                 ERbindingstatus = cscvarsEH[idx, "ER_binding"],
                                                 genomicLabel = cscvarsEH[idx, "Gene_symbol"],
...
table(sddf$BrCa4f, sddf$Agef, useNA = "always")
table(sddf$BrCa4, sddf$Agef, useNA = "always")/sum(table(sddf$BrCa4, sddf$Agef, useNA = "always"))*100
