...
simsmufit <- function(x, xdf = sddf, ydf = NULL, xvar = "age_at_diagnosis",
  ...
  xlbc <- "Dx age (years)"
  ylbfnc <- toupper(strsplit(x, split = "\\.")[[1]][2])
  ylbc <- paste(toupper(strsplit(x, split = "\\.")[[1]][2]), "(", ERbindingstatus, ")")
  ylbnc <- paste(subgroup, ": ", length(xdf[valdx, xvar]), "cases")

  sslbc <- tolower(strsplit(x, split = "\\.")[[1]][1]) ## score scale
  
  if ( pdfOutFilep ) {
      fnm <- gsub(" ", "_", paste(pdfFileNamePrefix, ylbfnc, "_", sslbc, ".pdf", sep = ""))
...
pdf(file = "NormalBreast13_IHC_Trends_95p0pct_2kSimCIs_shaded_v02.pdf", width = 8, height = 8)
par(mfrow = c(2, 2))
sapply(cscvars, simsmufit, nsim = 2000, alpha = 0.05)
dev.off()


date()
plDrNm <- "./Plots/NormalBreast13_IHC_Trends_Individual"
if ( !file.exists(plDrNm) ) { dir.create(plDrNm) }
set.seed(743); sapply(cscvars, simsmufit, xdf = sddf, nsim = 2000, alpha = 0.05,
                      pdfOutFilep = TRUE, plotDirName = plDrNm,
                      pdfFileNamePrefix = "NormalBreast13_IHC_Trends_95pct_2kSimCIs_shaded_")
date()


pdf(file = "NormalBreast13_IHC_Trends_99p0pct_10kSimCIs_shaded_v02.pdf", width = 8, height = 8)
par(mfrow = c(2, 2))
sapply(cscvars, simsmufit, nsim = 10000, alpha = 0.01)
dev.off()
...
if ( !file.exists(plDrNm) ) { dir.create(plDrNm) }
icinm <- "All cases"
set.seed(753); sapply(seq(nrow(cscvars)),
                      function(idx) {
                          simsmufit(cscvars[idx, 1], xdf = sddf,
                                    nsim = 10000, alpha = 0.01,
                                    pdfOutFilep = TRUE, plotDirName = plDrNm,
                                    pdfFileNamePrefix =
                                      paste("NormalBreast13_IHC_Trends_99p0pct_10kSimCIs_shaded_",
...
plDrNm <- "./Plots/NormalBreast13_IHC_Trends_Individual"
if ( !file.exists(plDrNm) ) { dir.create(plDrNm) }
set.seed(763); sapply(cscvars, simsmufit, xdf = sddf, nsim = 40000, alpha = 0.001,
                      pdfOutFilep = TRUE, plotDirName = plDrNm,
                      pdfFileNamePrefix = "NormalBreast13_IHC_Trends_99p9pct_40kSimCIs_shaded_")
date()



pdf(file = "NormalBreast13_IHC_Trends_99p99pct_400kSimCIs_shaded_v02.pdf", width = 8, height = 8)
par(mfrow = c(2, 2))
sapply(cscvars, simsmufit, nsim = 400000, alpha = 0.0001)
dev.off()
...
plDrNm <- "./Plots/NormalBreast13_IHC_Trends_Individual"
if ( !file.exists(plDrNm) ) { dir.create(plDrNm) }
set.seed(773); sapply(cscvars, simsmufit, xdf = sddf, nsim = 400000, alpha = 0.0001,
                      pdfOutFilep = TRUE, plotDirName = plDrNm,
                      pdfFileNamePrefix = "NormalBreast13_IHC_Trends_99p99pct_400kSimCIs_shaded_")
date()
...
write.csv(sddf, file = "TableS5_NormalBreast13_TMA_raw_data.csv")
